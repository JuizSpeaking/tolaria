...
describe('extractFrontmatterTitleFromContent', () => {
  ...
  it('extracts quoted frontmatter titles', () => {
    const content = '---\ntitle: "Spring 2026"\n---\n## Body'
    expect(extractFrontmatterTitleFromContent(content)).toBe('Spring 2026')
  })
  ...
  it('returns null for non-scalar frontmatter titles', () => {
    expect(extractFrontmatterTitleFromContent('---\ntitle: [One, Two]\n---\n## Body')).toBeNull()
    expect(extractFrontmatterTitleFromContent('---\ntitle: |\n  Long title\n---\n## Body')).toBeNull()
  })
})
